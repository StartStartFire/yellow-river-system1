...
> **定位**：本文件是前端**接入状态唯一出处**——各页面数据来源（前端 Mock / 后端 API / WebSocket）以本文件为准，代码为最终真相。需求定位与联动规则见 `docs/requirements/system-requirements.md`，页面实现细节见 `docs/page-design/pages-reference.md`。
...
业务数据部分来自 `src/mock/`（首页/基础数据/水调水情/案例库/报表统计/模型配置 Step 1-5），部分已接入后端真实 API 与 WebSocket（`src/api/index.ts`：配置汇总提交、过程透明 WS、评价决策）。不接数据库；MATLAB 由后端经 MATLAB Engine 调用，前端不直连。
...
> 详细规则见 `CLAUDE.md` 第 5 章「代码质量与架构原则」和第 22 章「代码复用强化策略」。
...
| 8  | 过程透明  |  ✅  | 方案切换 + 6 图表 + WebSocket 实时进度/过程数据 + 底部摘要                          |
| 9  | 评价决策  |  ✅  | Tab 导航（评价分析 / 决策分析），雷达图+桑基图+帕累托+排名（已接 /evaluate、/decision 真实 API） |
...
| `src/api/index.ts`（postRun）                                  | Step 6 配置汇总数据（Mock 已删除，组装参数调用 `POST /run`）       |
...
> **流水账历史**：已归档至（项目根）`docs/archive/SESSIONS.md`（每次会话记录，滚存近 5 次）

***

## 🏗️ 架构原则与核心决策

四项要求：**模块化 / 低耦合 / 可持续维护 / 可复用**。详见 `CLAUDE.md` 第 5 章。
