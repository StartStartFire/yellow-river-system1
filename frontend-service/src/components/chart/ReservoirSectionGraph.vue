...
const DAM_FACE_X = 1362

const reservoirClipPath = computed(() => {

  // 矩形裁剪：覆盖整个坝面左侧区域，背景图（z-index:2）的
  // 不透明像素会自动遮挡多余水体，透明区域显示水体
  const pts = [
    `M -80,0`,
    `L ${DAM_FACE_X},0`,
    `L ${DAM_FACE_X},654`,
...
const wavePath1 = computed(() => {
  ...
  return `M 350 ${baseY} Q 420 ${baseY - 4} 490 ${baseY} T 630 ${baseY} T 770 ${baseY}`
})
const wavePath2 = computed(() => {
  const wy = waterY.value
  const baseY = Math.min(wy + 14, 647)
  return `M 400 ${baseY} Q 470 ${baseY - 3} 540 ${baseY} T 680 ${baseY}`
})
</script>
