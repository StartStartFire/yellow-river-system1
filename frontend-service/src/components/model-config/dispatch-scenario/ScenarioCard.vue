    ...
    <div class="sub-options">
      <div
        ...
        :class="{
          ...
          'sub-placeholder': sub.status === 'placeholder',
        }"
        :style="sub.status === 'placeholder' ? { cursor: 'not-allowed', opacity: 0.45 } : {}"
        @click.stop="sub.status === 'placeholder' ? undefined : handleSubOptionClick(sub.id)"
      >
        <!-- radio 圆点 -->
        ...
        <!-- 占位标记 -->
        <div v-if="sub.status === 'placeholder'" class="placeholder-tag" style="font-size:10px;color:var(--tech-text-placeholder);background:rgba(255,255,255,0.05);padding:1px 8px;border-radius:8px;white-space:nowrap;">待开放</div>

        <!-- 小图标 -->
        <div v-if="subIconMap[sub.id] && sub.status !== 'placeholder'" class="sub-icon" v-html="svgIcons[subIconMap[sub.id] || '']"></div>
      </div>
    </div>
  </div>
</template>
