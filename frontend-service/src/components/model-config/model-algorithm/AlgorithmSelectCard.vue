...
const selectAlgo = (id: string) => {
  const algo = props.algorithms.find(a => a.id === id)
  if (algo?.status === 'placeholder') return
  emit('update:algorithmId', id)
}
</script>
    ...
    <div class="card-body">
      <div class="algo-options">
        <div
          ...
          class="algo-option"
          :class="{
            active: algorithmId === algo.id,
            'option-placeholder': algo.status === 'placeholder',
          }"
          :style="algo.status === 'placeholder' ? { cursor: 'not-allowed', opacity: 0.45 } : {}"
        ...
        >
          ...
          <span v-if="algo.status === 'placeholder'" class="placeholder-tag" style="font-size:10px;color:var(--tech-text-placeholder);background:rgba(255,255,255,0.05);padding:1px 8px;border-radius:8px;white-space:nowrap;margin-left:auto;">待开放</span>
        </div>
      </div>
    </div>
  </div>
