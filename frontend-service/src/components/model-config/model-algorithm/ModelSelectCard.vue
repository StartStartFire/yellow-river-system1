...
const selectModel = (id: string) => {
  ...
  if (model?.status === 'placeholder') return
      ...
      <div class="model-options">
        <div
          ...
          :class="{
            ...
            'option-placeholder': model.status === 'placeholder',
          }"
          :style="model.status === 'placeholder' ? { cursor: 'not-allowed', opacity: 0.45 } : {}"
        ...
        >
          ...
          <span v-if="model.status === 'placeholder'" class="placeholder-tag" style="font-size:10px;color:var(--tech-text-placeholder);background:rgba(255,255,255,0.05);padding:1px 8px;border-radius:8px;white-space:nowrap;margin-left:auto;">待开放</span>
        </div>
      </div>
    </div>
  </div>
