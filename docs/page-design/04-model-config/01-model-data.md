...
├──────────────────────────────────────────────────────────────┤
│ 模型配置流程步骤条：模型数据 → 基础配置 → 模型算法 → 场景配置 → 配置汇总 │
├───────────────┬──────────────────────────────────────────────┤
│ 左侧数据目录   │ 右侧内容区                                   │
│               │ 顶部操作区：上传数据 / 下载模板              │
│ 调度输入       │                                              │
...
│               │                                              │
├───────────────┴──────────────────────────────────────────────┤
│ 底部操作栏（ModelConfigFooter）：取消 / 保存 / 下一步          │
└──────────────────────────────────────────────────────────────┘
```
...
3. 点击"确认取消"后关闭弹窗，在窗口顶部以 toast 浮动提示"已取消，未保存任何更改"。
4. 点击"继续编辑"关闭弹窗，停留在当前页面。
5. 不清空真实数据，不调用接口。
6. 取消确认后的提示以 ElMessage toast 形式显示在窗口顶部居中位置，不在页面底部显示。
```
