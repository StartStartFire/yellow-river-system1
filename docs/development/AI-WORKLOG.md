...
3. 全局样式系统新增（src/styles/index.css）：
   - .page-panel：页面级统一面板（background: rgba(6,20,42,0.88); backdrop-filter: blur(16px)）
   - .h-divider / .v-divider：发光渐变分割线（linear-gradient，中心 rgba(0,212,255,0.3)，两端透明）
...
4. 所有 8 个页面 + 模型配置 6 个子页面样式升级：
   - 移除所有独立卡片背景（background: rgba(6,30,70,...)）
   - 移除所有卡片边框（border: 1px solid rgba(50,150,255,...)）
   - 移除所有 border-radius（卡片类型元素）
   - 移除所有 box-shadow（卡片类型元素）
...
src/components/home/ReservoirMonitorPanel.vue                         — 首页水情监控分区
src/components/common/PanelCard.vue                                   — 通用分区面板组件（原卡片组件重构）
