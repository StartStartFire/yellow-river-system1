...
│   ├── http_callback_push.m # HTTP 回调推送（webwrite 封装，静默降级）
│   ├── push_callback_data.m # 回调数据统一封装（progress 汇总指标 + process_data 过程数据，被两个主循环每 10 代调用）
│   ├── main.m               # 程序入口
...
│   │   ├── main_controller.py    # 主控制器（单算法/全算法/整合）
...
1. **低耦合**：模块间通过明确的接口（函数签名、配置文件、消息队列）通信，不共享全局可变状态
2. **高内聚**：每个模块只负责一个职责，职责边界清晰，不跨域
3. **可复用**：通用逻辑抽取为独立函数/类，避免代码重复
4. **可持续维护**：代码注释说明"为什么"而非"是什么"，复杂逻辑必须有文档或伪代码说明

### 复杂任务处理

1. **Sub-agent 拆分**：当一个任务涉及多个领域（如同时修改 MATLAB 和 Python 代码）、或多个文件、或需先调研再决策时，应将调研/分析工作委托给 sub-agent（使用 Agent 工具），本 agent 聚焦于决策和实现
2. **先读后写**：对不熟悉的代码段，先委托 sub-agent 阅读分析并给出摘要，再决定修改方案
3. **变更记录**：每次 sub-agent 完成工作后，将关键结论（文件路径、函数名、依赖关系）记录到 memory，便于后续步骤查阅

### 沟通规范

1. **方案先行**：对不确定的实现方案，先询问用户确认，再动手编码
2. **透明进度**：多步任务执行时，每步完成后同步进度和下一步计划

### 通用原则

1. **Web 服务目录与模型目录严格分离**：MATLAB 文件归 `matlab-model/`，Python 文件归 `backend-service/`，不交叉存放
2. **不改 MATLAB 模型核心逻辑**：新增 `http_callback_push.m`（webwrite 推送）与 `push_callback_data.m`（回调数据统一封装，被两个主循环每 10 代调用）作为仅有的"侵入点"，不修改 `evaluate_objective_*`、`load_data` 等核心计算文件（除非修 Bug）
3. **静默降级原则**：Web 服务的任何故障不影响 MATLAB 模型的正常运行。回调推送失败时应当 try-catch 静默降级，数据仍通过原有的 JSONL 文件机制持久化
4. **临时测试规范**：AI 和开发者的快速验证脚本一律放在根目录 `_sandbox/` 下（已在 `.gitignore` 中），用完及时删除；禁止在 `_sandbox/` 之外的目录创建临时测试文件
