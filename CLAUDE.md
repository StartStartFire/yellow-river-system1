...
**当前阶段：功能扩展** — 原型阶段（6 步）已完成，进入功能扩展期：Web 服务模块化重构完成、评价系统（NMF/PP/AHP-FUZZY）集成完成、约束参数动态注入完成、过程透明化链路待实施、前后端联调待实施。
...
| 方面 | 原型期做法 | 后续阶段 |
|------|-----------|---------|
| 配置 | `config.py` dataclass，改文件即可 | 环境变量 / `.env` 覆盖 |
| 类型注解 | 函数签名标注类型，内部变量从简 | 全量 PEP 484 |
| 错误处理 | 关键路径 try-catch，非关键路径直接抛 | 结构化错误响应 |
| 测试 | 手动 curl / WebSocket 客户端验证 | 单元测试 + 集成测试 |
| 输入校验 | Pydantic 校验 API 请求和回调数据 | 同上 + 更严格的校验规则 |

### 坚持（原型期也不能妥协）

- 目录分离（MATLAB / Python 不交叉存放）
- 模块职责单一（一个文件只做一件事）
- 配置收拢到 `config.py`，业务代码不出现硬编码路径
...
2. **类型注解**：函数参数和返回值标注类型，内部变量从简（原型阶段）
...
| [分步实施计划](docs/development-steps.md) | 原始 6 步，✅ 已全部完成 |
| [Web 服务化技术方案](docs/technical-roadmap.md) | 整体架构设计参考 |
